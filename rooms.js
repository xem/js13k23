...
//if(debug) { inv=["telescope", "lit torch","hammer","gear","boulder","crown","gear","gear","mouse","wand","sword","key"]; draw_inv() }
...
windowright = (bars,tower,cl='') => {
  C.plane({x:300-70,y:-95+92,z:-50,w:100,h:100,html:(r40c > 60 && r40c < 300) ? svgwindownight : svgwindow,css:"window",sx:-1,ry:tower?-10:0,cl});
...
draw_sky = (leftwindow,rightwindow,hay) => {
  ...
  C.group({n:"skygroup",x:0,y:200,rz:r40d});
  ...
  else if(hay && rightwindow && r40c == 60 && r21b && !r21g){
...
room00 = () => {
  ...
  if(!r00d)C.plane({x:145,y:-40,w:20,h:15,html:svghanger,css:"hanger",n:"r002",cl:"r00c"});
  
  if(!r00f){
    C.plane({x:-105,y:120,z:20,html:"🔒",css:"lock",rz:-2,cl:"r00e",n:"r003"});
    C.plane({x:-137,y:110,html:"🚪",css:"door",ry:5,z:10,n:"r004",o:"100px 100px"});
  }
  else {
...
r00e = () => {
  if(inv[using] == "hanger"){
    ...
    C.move({n:"r004",x:-110,y:115,z:15,ry:120}); // door
    rem_inv("hanger");
    r00f = 1;
  }
}
...
room12 = (i,j) => {
  ...
  for(i=0;i<3;i++){
    for(j=0;j<4;j++){
      if(!(j>0&&!((i+j-3)%3)))C.plane({x:i*32-35 + Math.random()*10,y:j*65-30,w:20+Math.random()*2,h:45,b:`hsl(${(i*5+j*15)*20} 50% ${63}%)`,z:20,css:"book",cl:"book"+i+j});
    }
  }
...
room23 = () => {
  ...
  if(r23a) C.plane({x:155-5+15,y:-10+30,html:"⚙️",css:"gear",n:"g1"});
  if(r23c) C.plane({x:120-4+15,y:-40+30,html:"⚙️",css:"gear",n:"g2"});
  if(r23e) C.plane({x:155-5+15,y:-70+30,html:"⚙️",css:"gear",n:"g3"});
  if(r23g) C.plane({x:157+15,y:75,z:-63,html:"🗡️",css:"sword",ry:295,cl:"r23j",n:"r239"});
}

// Put gear 1
r23a = 0;
r23b = () => {
  if(usingstr == "use gear with " && !r23a){
    rem_inv("gear");
    use(-1);
...
r23c = 0;
r23d = () => {
  if(usingstr == "use gear with " && !r23c){
    rem_inv("gear");
    use(-1);
...
r23e = 0;
r23f = () => {
  if(usingstr == "use gear with " && !r23e){
    rem_inv("gear");
    use(-1);
...
r23g = 0;
r23h = () => {
  if(usingstr == "use sword with " && !r23g){
    rem_inv("sword");
    use(-1);
...
room23c = () => {
  ...
  setTimeout("inventory.style.opacity=0;C.move({n:'db',rx:90})",1500);
...
room32 = () => {
  ...
  C.plane({x:-15,y:-120,z:-50,w:950,h:50,b:"linear-gradient(#333,#555)",rx:145});
  C.plane({x:-15,y:235,z:-50,w:950,h:70,b:"linear-gradient(#333,#555)",rx:60});
  if(!r32a)C.plane({x:-120,y:230,w:30,h:30,html:"🔨",css:"hammer",on:"",rz:-45,n:"r323"});
...
room33 = () => {
  ...
  if(!r33c){
    if(r33b) C.plane({x:130,y:60,html:"🍲",css:"hot soup",n:"r331"});
...
// Talk to cook
r33a = () => {
  note.innerHTML = (usingstr == "use ht soup with " || (r33b && !r33c)) ? "Thanks! You can take the cheese!" : "Please help me heating up the soup!"
  note.className = "";
}
...
room40 = () => {
  ...
  C.group({n:"wheel",x:-175,y:15,w:150,h:150,on:"time wheel",css:"timewheel",o:"center center",rz:r40d});
...
r40a = () => {
  if(usingstr == "use wand with " || r40b){
    r40b = 1;
    rem_inv("wand");
...
update_sky = () => {
  //console.log(r40c);
  if(r40c == -60) { r40c = 360-60; day--; phase --;}
  if(r40c >= 360) { r40c -= 360; day++; phase++; }
  phase = day % 8;
  moonstr=[..."🌑🌒🌓🌔🌕🌖🌗🌘"][phase];
  if(r40c == 240 || r40c == 120){ C.move({n:"moon",y:650}) }
  else { C.move({n:"moon",y:600}) }
  
  
  if(r40c <= 60 || r40c > 240) viewport.style.background = "#cde";
  if(r40c > 60 && r40c <= 240) viewport.style.background = "#002";
  C.move({n:"skygroup",rz:r40d});
  try { moon.innerHTML = moonstr; wheelmoon.innerHTML = moonstr; }
  catch(e) {}
  
}

r40r = () => { r40c += 60; r40d += 60; C.move({n:"wheel",rz:r40d}); update_sky(); }
r40l = () => { r40c -= 60; r40d -= 60; C.move({n:"wheel",rz:r40d}); update_sky(); }
...
room41 = () => {
  ...
  else if(day < 796 || (day == 796 && r40c < 120)){
...
room42 = () => {
  ...
  if(!r42pf) { C.plane({x:-262,y:145,z:10,html:"🚪",css:"door"}); }
