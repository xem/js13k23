...
acs=[];
song = (G,D,i,O,a,s,A) => {
  a=(notes,center,duration,decaystart,decayduration,interval,volume,i)=>{
      ...
      with(G=createGain())
        for(i=0;i<notes.length;i+=2){
          with(O=createOscillator()){
            ...
            start(notes[i]*interval),
            frequency.setValueAtTime(center*1.06**(13-notes[i+1]),notes[i]*interval),
            type="triangle",
            gain.setValueAtTime(volume,notes[i]*interval),
            gain.setTargetAtTime(1e-5,notes[i]*interval+decaystart,decayduration),
            stop(notes[i]*interval+duration);
          }
       }
    }
  }
  ...
  a([0,21,4,21,5,21,6,21,9,21,12,21,15,21,16,21,17,21,18,21,21,21,24,21,27,21,28,21,29,21,30,21,33,21,36,21,39,21,40,21,41,21,42,21,45,21,48,21,51,21,52,21,53,21,54,21,57,21,60,21,63,21,64,21,65,21,66,21,69,21,72,21,75,21,76,21,77,21,78,21,81,21,84,21,87,21,88,21,89,21,90,21,93,21,96,21,99,21,100,21,101,21,102,21,105,21,108,21,111,21,112,21,113,21,114,21,117,21,120,21,123,21,124,21,125,21,126,21,129,21,132,21,135,21,137,21,136,21,138,21,141,21,144,21,147,21,148,21,149,21,150,21,153,21,3,21],135,.11,.05,.05,.17,.05);
  a([0,9,12,9,0,14,12,14,24,9,24,14,36,9,36,14,0,2,12,2,36,2,24,2],100,.5,.1,.3,.17,.05);
